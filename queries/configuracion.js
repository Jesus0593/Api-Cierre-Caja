...
WHERE IMPORTE<>0 OR DECLARADO<>0 OR CODFORMAPAGO=''1''
    ...
    `,
    `
    IF  NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ripAppWeb].[RIP_COBROS_FRONT]') AND type IN (N'TF'))
BEGIN 
EXEC ('CREATE FUNCTION [ripAppWeb].[RIP_COBROS_FRONT](@REDONDEO AS INT, @FECHA AS DATE, @CAJA AS NVARCHAR(4), @Z AS INT,
...
AS BEGIN
	...
	SET @DEVOLUCIONES = 0 --ISNULL((SELECT ISNULL(SUM(FV.TOTALNETO*FV.FACTORMONEDA),0) FROM FACTURASVENTA FV WHERE FV.FECHA=@FECHA AND FV.CAJA LIKE @CAJA AND FV.Z=@Z AND FV.TOTALNETO<0),0)
	--SELECT @DEVOLUCIONES
