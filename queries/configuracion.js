...
BEGIN
EXEC ('
CREATE FUNCTION [ripAppWeb].[COSTOVENTA] (@REDONDEO AS INT, @CAJA AS NVARCHAR(4), @Z AS INT ,@FECHA AS DATE)
RETURNS @TABLA TABLE
(
...
END
	`,
	`
	IF  NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ripAppWeb].[RIP_IMPUESTOSFRONT]') AND type IN (N'TF'))
BEGIN
EXEC ('
CREATE FUNCTION [ripAppWeb].[RIP_IMPUESTOSFRONT](@REDONDEO AS INT, @FECHA AS DATE , @CAJA AS NVARCHAR(4), @Z AS INT)
RETURNS 
...
SELECT
	...
	''CIERRE DE '' + UPPER(S.DESCRIPCION) + '' DEL '' + CONVERT(char(10), @FECHA, 103) + ''Z - '' + CONVERT(varchar, @Z)  COLLATE Latin1_General_CS_AI COMENTARIO,
	CASE
		WHEN ROUND(SUM(AVL.TOTAL * AVC.FACTORMONEDA * (100-AVC.TOTDTOCOMERCIAL)/100  * AVL.IVA/100),@REDONDEO) < 0 
		THEN ROUND(SUM(AVL.TOTAL * AVC.FACTORMONEDA * (100-AVC.TOTDTOCOMERCIAL)/100  * AVL.IVA/100),@REDONDEO) * -1
		ELSE 0
	END DEBE,
	CASE
		WHEN ROUND(SUM(AVL.TOTAL * AVC.FACTORMONEDA * (100-AVC.TOTDTOCOMERCIAL)/100  * AVL.IVA/100),@REDONDEO) >= 0 
		THEN ROUND(SUM(AVL.TOTAL * AVC.FACTORMONEDA * (100-AVC.TOTDTOCOMERCIAL)/100  * AVL.IVA/100),@REDONDEO) 
		ELSE 0
	END HABER
	, ALM.CENTROCOSTE COLLATE Latin1_General_CS_AI CENTROCOSTE
FROM 
